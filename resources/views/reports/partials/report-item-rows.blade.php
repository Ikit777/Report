...
<tbody id="reportItemRows">
    @for($i = 0; $i < $itemRowCount; $i++)
        ...
        <tr>
            ...
            <td><input type="text" name="items[{{ $i }}][petugas_pagi]" class="sheet-input" value="{{ old("items.{$i}.petugas_pagi", $savedItem?->petugas_pagi ?? Auth::user()->name) }}"></td>
            ...
            <td><input type="text" name="items[{{ $i }}][petugas_sore]" class="sheet-input" value="{{ old("items.{$i}.petugas_sore", $savedItem?->petugas_sore ?? Auth::user()->name) }}"></td>
